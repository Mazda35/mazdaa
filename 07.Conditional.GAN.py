...
from keras.layers import Embedding,Multiply,Concatenate,Input
...
def cGenerativeFn(zDim):

    z=Input(shape=(zDim,))
    label=Input(shape=(1,),dtype='int32')

    # (batch_size,1,100)
    labelEmbed=Embedding(numClasses,zDim,input_shape=1)(label)
    # (batch_size,100)
    labelEmbed=Flatten()(labelEmbed)

    joinedRepresentation=Multiply()([z,labelEmbed])
    # Generative function produce an image
    genVariable=generativeFn(zDim)
    conditionalImage=genVariable(joinedRepresentation)

    return Model([z,label],conditionalImage)
...
def cDiscriminativeFn(imageShape):

    image=Input(shape=imageShape)
    label=Input(shape=(1,),dtype='int32')

    # (batch_size,1,28*28*1)
    labelEmbed=Embedding(numClasses,np.prod(imageShape),input_shape=1)(label)
    # (batch_size,28*28*1)
    labelEmbed=Flatten()(labelEmbed)

    labelEmbed= Reshape(imageShape)(labelEmbed)
    # We join image and label from last dimension
    concateImage=Concatenate(axis=-1)([image,labelEmbed])
    disVariable=discriminativeFn(imageShape)
    classification=disVariable(concateImage)

    return Model([image,label],classification)

# cGan Function
def cGanFn(generator,discriminator):

    z=Input(shape=(zDim,))
    label=Input(shape=(1,),dtype='int32')

    fImage=generator([z,label])
    classification=discriminator([fImage,label])
...
disVariable=cDiscriminativeFn(imageShape)
